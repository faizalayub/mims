<?php
   ...
   class Users{
       ...
       public function __construct() {
        ...
        public function registerUserLogin($array){
            $usernameKey  = $array["username"];
            $userpassKey  = password_hash($array["password"], PASSWORD_DEFAULT);
            $userfnameKey = $array["fullname"];
            $result_data = [];

            $result_data['status'] = $this->conn->prepare("INSERT INTO `user` (`id`, `user_id`, `user_name`, `user_department`, `user_password`, `createdAt`, `role_id`, `location`) VALUES (NULL, '".$usernameKey."', '".$userfnameKey."', 'TM Tower', '".$userpassKey."', current_timestamp(), '1', '1')")->execute();
       ...
       public function addItem($array){
        ...
        if($result_check-> rowCount() > 0){
           ...
           $sql_add = "INSERT INTO item SET category_id = :category_id, brand_id = :brand_id, serial_no = NULL , model_id = :model_id, createdBy = :createdBy, region_current = :region_id, region_from = :region_id, status_id = :status_id";
           $result = $this->conn->prepare($sql_add);
           $result->execute($args_add);
           if($result){
               $result_data["valid"] = true;
...
$args = array("location" => $location);
   ...
   public function getFaultyRecordOnLoad($location){
       ...
       LEFT JOIN region ON region.region_id = item_status.is_region_current
       LEFT JOIN user ON user.location = region.region_id
       WHERE item_status.is_status_id = 3
       GROUP BY item.item_id       
       ORDER BY item_id DESC";
